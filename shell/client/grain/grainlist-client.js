...
Template.sandstormGrainListPage.events({
  "click .grain-list .question-mark": function (evt) {
  ...
  "input .search-bar": function (evt) {
    Template.instance()._filter.set(evt.target.value);
  },

  "keypress .search-bar": function (evt, instance) {
    if (evt.keyCode === 13) {
  ...
  "click button.empty-trash": function (evt, instance) {
  ...
  "click button.show-trash": function (evt, instance) {
    Router.go("grains", {}, { hash: "trash" });
  },

  "click button.show-main-list": function (evt, instance) {
    Router.go("grains", {}, { hash: "" });
  },

  "click .restore-button": function (evt, instance) {
...
Template.sandstormGrainTable.events({
  "click tbody tr.action": function (evt) {
    this && this.onClick();
  },

  "click tbody tr.grain .click-to-go": function (evt) {
    const context = Template.instance().data;
    context.onGrainClicked && context.onGrainClicked(this._id);
  },

  "click .select-all-grains>input": function (evt, instance) {
    if (instance._numMineSelectedShown.get() == 0 &&
        instance._numSharedWithMeSelectedShown.get() == 0) {
      // select all
  ...
  "change .select-grain.mine>input": function (evt, instance) {
    evt.preventDefault();
  ...
  "change .select-grain.shared>input": function (evt, instance) {
    if (evt.target.checked) {
  ...
  "click td.select-grain": function (evt, instance) {
  ...
  "click .bulk-action-buttons>button": function (evt, instance) {
