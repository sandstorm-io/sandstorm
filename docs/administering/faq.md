...
**Yes !** Let's Encrypt [recently started issuing wildcard certificates] (https://certbot.eff.org/faq/#does-let-s-encrypt-issue-wildcard-certificates), which are needed for Sandstorm
for all the reasons documented in [our documentation on wildcard DNS.](wildcard.md).

To make it all work, you can follow the instructions in [our documentation on reverse proxy](reverse-proxy.md) to configure nginx and Sandstorm. Just generate a certificate and a private key for both your base url (`example.com`) and the wildcard host you created (`*.example.com`).
