...
**MongoDB Note:**  If your app uses MongoDB, please use [Kenton's fork](https://github.com/kentonv/mongo/tree/niscu) rather than the original Mongo (be sure to check out the `niscu` branch, not `master`, from that repo).  Mongo is optimized for huge databases, but Sandstorm grains should be tiny.  Kenton's fork changes some hard-coded constants to make that possible.  Eventually we'd like to get this merged upstream in some form but there's no timeline for that happening.

Other databases may have similar problems.  Once your app is working, check disk usage under `/var/sandstorm/grains` to make sure your app's grains are of reasonable size.
