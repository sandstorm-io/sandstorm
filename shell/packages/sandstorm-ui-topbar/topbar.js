...
Template.sandstormTopbar.helpers({
  ...
  grains: function () {
    ...
    var data = grains.map(function (grain) {
      ...
      var appTitle;
      if (grain.tokenOnly) {
        // TODO(now): make this generated by the router
        grainTitle = grain.title;
        // TODO(soon): plumb through denormalized grain metadata for unredeemed ApiTokens?
        iconSrc = Identicon.identiconForApp("da39a3ee5e6b4b0d3255bfef95601890afd80709", "grain");
        appTitle = "(unknown app)";
      } else if (grain.token) {
        var token = ApiTokens.findOne({grainId: grain.grainId,
                                            "owner.user.userId": Meteor.userId()},
                                            {sort : {created : 1}});
        var tokenOwner = token && token.owner && token.owner.user;
        grainTitle = tokenOwner && tokenOwner.title;
        var grainMetadata = tokenOwner && tokenOwner.denormalizedGrainMetadata;
        iconSrc = grainMetadata && (grainMetadata.icon ? grainMetadata.icon.assetId : (
                  grainMetadata.appId ? Identicon.identiconForApp(grainMetadata.appId, "grain") :
                  Identicon.identiconForApp("da39a3ee5e6b4b0d3255bfef95601890afd80709", "grain")
                ));
        appTitle = grainMetadata.appTitle.defaultText; // TODO(someday): don't use defaultText
      } else {
        grainLink = "/grain/" + grain.grainId;
        var gr = Grains.findOne({_id: grain.grainId});
        grainTitle = gr && gr.title;
        var pkg = Packages.findOne({_id: gr.packageId});
        iconSrc = Identicon.iconSrcForPackage(pkg, "grain", topbar._staticHost);
        appTitle = pkg && pkg.manifest && pkg.manifest.appTitle && pkg.manifest.appTitle.defaultText;
        // TODO(someday): don't use defaultText
      }
      */
      return {
